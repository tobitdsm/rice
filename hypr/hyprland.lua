...
hl.monitor({
    output   = "HDMI-A-1",
    mode     = "1920x1080@60",
    position = "0x-1080",   -- on top
    -- position = "1920x0",    -- to the right
    -- position = "1920x-400", -- to the right on a flat surface
    -- position = "0x0",   -- "duplicate"
    scale    = "1",
})
...
local fileManager = "LC_COLLATE=\"C.UTF-8\" thunar"
local menu = "pgrep -x wofi >/dev/null 2>&1 || wofi --show drun" -- prevent multiple instances

-- custom
local taskManager = "gnome-system-monitor"
...
hl.on("hyprland.start", function () 
  hl.exec_cmd("ironbar")    -- start ironbar & load css
...
hl.on("config.reloaded", function ()
  hl.exec_cmd("gsettings set org.gnome.desktop.interface gtk-theme \"Nordic-darker\"")  -- dark mode for GTK3 apps
  hl.exec_cmd("gsettings set org.gnome.desktop.wm.preferences theme \"Nordic-darker\"")  -- dark mode for GTK3 apps
  hl.exec_cmd("gsettings set org.gnome.desktop.interface color-scheme \"prefer-dark\"")  -- dark mode for GTK4 apps
end)
...
hl.env("HYPRCURSOR_THEME", "moxxey-hyprcursor")
hl.env("HYPRCURSOR_SIZE", "32")
hl.env("XCURSOR_THEME", "moxxey-hyprcursor")
hl.env("XCURSOR_SIZE", "32")

hl.env("QT_QPA_PLATFORMTHEME", "qt6ct") -- dark mode for Qt apps

hl.env("WLR_RENDERER_ALLOW_SOFTWARE", "1")  -- GPU acceleration
...
hl.config({
    general = {
        ...
        col = {
            active_border = { colors = {"rgba(ff6666ff)", "rgba(996b6bff)"}, angle = 45 },
            inactive_border = { colors = {"rgba(996b6bff)", "rgba(ff6666ff)"}, angle = 45 },
        },
...
-- Default springs
hl.curve("easy",           { type = "spring", mass = 1, stiffness = 213.7899, dampening = 23.7410466 })
...
hl.config({
    misc = {
        ...
        disable_splash_rendering = true,    -- Disable the splash text
    },
})
...
hl.device({
    name        = "asue1213:00-04f3:3283-mouse",
    sensitivity = -0.6,
})
...
hl.bind(mainMod .. " + SPACE", hl.dsp.exec_cmd("pgrep -x rofi >/dev/null 2>&1 || find / -mount -type f | rofi -dmenu -i -p \"Open file\" | xargs -r xdg-open"))
hl.bind(mainMod .. " + CTRL + P", hl.dsp.window.pin({ action = "toggle" }))

hl.bind("CTRL + F11", hl.dsp.exec_cmd("grim -g \"$(slurp)\" - | wl-copy"))
